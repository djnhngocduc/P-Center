...
def search_min_radius_hybrid_threshold(
    ...
    *,
    ...
    Hybrid threshold search:
      - if remaining_iters > hybrid_threshold:
            * INTERNAL PySAT solver -> incremental SAT
            * EXTERNAL solver       -> normal per-radius SAT via _solve_radius_worker_proc
      - else: use CPLEX
    """
    search_t0 = time.perf_counter()
    ...
    # -------------------------------------------------
    # SAT-side = external solver -> normal SAT per-radius
    # -------------------------------------------------
    else:
        radii = inst.radii
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=str,
        default=os.path.join("results", "results", "threshold_detail.csv"),
...
def _remaining_iters_from_bounds(lo: int, hi: int) -> int:
    """
    Number of binary-search iterations remaining (including current step)
    on an interval [lo, hi] over sorted radii indices.
    """
    width = hi - lo + 1
    if width <= 1:
        return 0
...
def _solver_wall_cost(status: str, wall_time: float, time_limit: float) -> float:
    """
...
def profile_threshold_incremental_vs_cplex(
    ...
    *,
    ...
    """
    Profile threshold for hybrid strategy:
      - walk exactly one binary-search path
      - at each tested radius:
          * solve with incremental SAT (same incremental state as current project)
          * solve with CPLEX
      - group results by remaining_iters
      - simulate threshold policies:
            use SAT if remaining_iters > T, else use CPLEX
    ...
    with Solver(name=sat_solver_name, bootstrap_with=base_cnf.clauses) as solver:
        ...
        while lo <= hi:
            ...
            # winner by wall-time among valid sat/unsat solves
            winner = "tie"
            if sat_status in ("sat", "unsat") and cplex_status in ("sat", "unsat"):
            ...
            # Use SAT decision as the canonical binary path.
            # If SAT is not decisive, fall back to CPLEX.
    ...
    # Merge per-k summary and threshold simulation into one CSV-friendly list
    summary_export_rows = []
...
def run_experiment(
    ...
    *,
    ...
    if search_mode == "threshold_profile":
        ...
        prof = profile_threshold_incremental_vs_cplex(
            inst,
            encoding,
            sat_solver,
            time_limit,
...
def sort_key(enc_sol_mode):
    ...
    mode_rank = {"parallel": 0, "binary": 1, "kary": 2, "incremental": 3, "threshold_profile": 4, "hybrid_threshold": 5}
